  ...
  (when (and parent-node
    ...
    (when-let* ((current-node-type
      ...
      (unless (tsc-node-eq current-node first-sibling)
        (cdr (seq-elt (tsc-node-range first-sibling) 2))
        ;;; Bug: first-sibling-position will be 2 larger than it should be
        ;; (save-excursion
        ;;   (goto-char first-sibling-position)
        ;;   (message "%s %s %s" (line-beginning-position) first-sibling-position (tsc-node-range first-sibling))
        ;;   (- first-sibling-position
        ;;      (line-beginning-position)))
        ))))
       ...
       (lambda (current-node)
         (let* ((previous-node
                ...
                (same-line-with-parent-node-p
                 (when parent-node
                   (equal (car (seq-elt (tsc-node-range parent-node) 2))
                          (car (seq-elt (tsc-node-range current-node) 2)))))
                (same-line-with-prev-node-p
                 (when previous-node
                   (equal (car (seq-elt (tsc-node-range previous-node) 3))
                          (car (seq-elt (tsc-node-range current-node) 2)))))
...
(defun tree-sitter-indent-line-and-debug ()
  ...
  (let* ((line-str (thing-at-point 'line))
    ...
    (message "Parentwise path of ⎡%s⎦ (while looking at ⎡%s⎦)"
    ;; (message "tree-sitter-indent: Indented ⎡%s⎦ to ⎡%s⎦ (col %s) because of parentwise path of ⎡%s⎦ (while looking at ⎡%s⎦)"
             ;; line-str
             ;; (thing-at-point 'line)
             ;; column
             readable-parentwise-path
