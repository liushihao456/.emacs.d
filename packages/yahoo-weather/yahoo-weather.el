...
(defcustom yahoo-weather-locationid "1816670"
  "Location ID."
  :type 'string
  :group 'yahoo-weather)

(defcustom yahoo-weather-apikey "e5f1e557685536c2fe98e7dfabe201de"
  "Api Key."
  :type 'string
  :group 'yahoo-weather)
...
(defun yahoo-weather-get-query-url (locationid apikey)
  "Generate url that used to fetch weather information."
  (let* ( (url (format "http://api.openweathermap.org/data/2.5/weather?id=%s&appid=%s" locationid apikey))) url)
  )
...
(defun yahoo-weather-info-format (json-object format-string)
  (let* (
	 ;; (temperature-K (string-to-number (yahoo-weather--extract-from-json-object json-object '(main temp))))
         (temperature-K  (yahoo-weather--extract-from-json-object json-object '(main temp)))
	 (temperature (yahoo-weather--k_to_c temperature-K))
	 (text (yahoo-weather--extract-from-json-object (elt (alist-get 'weather json-object) 0) '(main)))
	 ...
	 (wind-speed (yahoo-weather--extract-from-json-object json-object '(wind speed)))
	 (atmosphere-humidity (yahoo-weather--extract-from-json-object json-object '(main humidity)))
	 (atmosphere-pressure (yahoo-weather--extract-from-json-object json-object '(main pressure)))
	 ;; (atmosphere-rising (yahoo-weather--extract-from-json-object json-object '(query results channel atmosphere rising)))
	 (atmosphere-visibility (yahoo-weather--extract-from-json-object json-object '(visibility)))
	 (sunrise-time (yahoo-weather--extract-from-json-object json-object '(sys sunrise)))
	 (sunset-time (yahoo-weather--extract-from-json-object json-object '(sys sunset)))
	 ;; (temperature-K (string-to-number (yahoo-weather--extract-from-json-object json-object '(query results channel item condition temp))))
	 ;; (temperature (yahoo-weather--k_to_c temperature-K))
	...
	(setq format-string (replace-regexp-in-string "%(temperature)" (format yahoo-weather-temperture-format temperature-K) format-string t))
    ...
    (setq format-string (replace-regexp-in-string "%(sunrise-time)" (format-time-string "%H:%M" sunrise-time) format-string t))
    (setq format-string (replace-regexp-in-string "%(sunset-time)" (format-time-string "%H:%M" sunset-time) format-string t))
    format-string))
...
(defun yahoo-weather-update-info ()
  ...
  (let ((url (yahoo-weather-get-query-url yahoo-weather-locationid yahoo-weather-apikey)))
    (url-retrieve url #'yahoo-weather-update-info-cb nil t))
  )

(defun yahoo-weather--k_to_c (temp)
